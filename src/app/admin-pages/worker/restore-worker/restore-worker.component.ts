...
@Component({
  ...
  styleUrls: ['./restore-worker.component.css']
})
export class RestoreWorkerComponent implements OnInit {

//////////////////////////////////// Tabel Angular Material /////////////////////////////////
@ViewChild(MatPaginator,{static: true}) paginator: MatPaginator;
displayedColumns: string[] = ['select', 'name','salary', 'title'];
selection = new SelectionModel(true, []);
filter = '';
dataSourceSearchTabel: any;

/////////////////////////////////// Lables ///////////////////////////////////////////
workerRestored: string;
...
ngOnInit() {
  ...
  this._workerService.getSelectDeleted().subscribe(response => {
...
restoreWorker() {
  ...
  for (let i = 0; i < this.selectedArrayValues.length; i++) {
    ...
    this._workerService.restoreWorker( this.selectedArrayValues[i]).subscribe(response => {
      let result = response
      if (result.message === 'the worker is restored') {
        this.successfullFlag = true;

        this.workerRestored = `<i class="fas fa-check-circle text-success" style="font-size: 60px"></i>
        <span class="d-block text-center" style="font-size: 18px;">Worker restored ${this.selectedArrayValues.length} successfully  </span>`;
        $('.responseMessage').html(this.workerRestored); // Mohammed hesham
  ...
  if (this.selectedArrayValues.length < 1) {
    ...
    this.workerRestored = `<i class="fas fa-info-circle text-warning" style="font-size: 60px"></i>
  ...
  reload() {
   ...
   this.router.navigateByUrl('/Dashboard', {skipLocationChange: true})
     .then(() => this.router.navigate(['Dashboard/worker/restoreWorker']));
    }, 1000);
 }
